...
class TestConfigBig(TestConfig):
    ...
    def test_stats(self):
        ...
        #----------------------------------------------------------------
        # schedule a downtime of 10 minutes for the hosts and srevices
        #----------------------------------------------------------------
        duration = 6000
        now = int(time.time())
        end = now + duration
        ...
        query = """GET downtimes
Columns: host_name author comment start_time end_time duration
Filter: is_service = 0
OutputFormat: python
"""

        expected_result = [
	    ['test_host_005', 'test_contact', 'dth', now, end, duration],
            ['test_host_009', 'test_contact', 'dth', now, end, duration]
        ]
        self.execute_and_assert(query, expected_result)

        query = """GET downtimes
Columns: host_name service_description author comment start_time end_time duration
Filter: is_service = 1
OutputFormat: python
"""

        expected_result = [
            ['test_host_001', 'test_warning_19', 'test_contact', 'dts', now, end, duration],
            ['test_host_003', 'test_warning_03', 'test_contact', 'dts', now, end, duration],
            ['test_host_005', 'test_warning_02', 'test_contact', 'dts', now, end, duration],
            ['test_host_000', 'test_critical_03', 'test_contact', 'dts', now, end, duration],
            ['test_host_005', 'test_critical_11', 'test_contact', 'dts', now, end, duration],
            ['test_host_007', 'test_critical_02', 'test_contact', 'dts', now, end, duration],
            ['test_host_008', 'test_critical_13', 'test_contact', 'dts', now, end, duration]
        ]
        self.execute_and_assert(query, expected_result)
